---
layout: post
title: Tutorial for creating Jupyter notebook widget
tags:
- python
- jupyter
- notebook
...
The Jupyter widget that we'll create in this example will allow us to add [Britecharts](http://eventbrite.github.io/britecharts/) to our Jupyter notebook.
...
Also, you can go over the source code of these ipwidgets such as [ipyleaflet](https://github.com/ellisonbg/ipyleaflet) or [gmaps](https://github.com/pbugnion/gmaps) and learn from the code how to create Jupyter widgets.

Another tutorial will show how to [embed mapbox plots in Jupyter notebooks](https://www.ryanbaumann.com/blog/2016/4/3/embedding-mapbox-plots-in-jupyter-notebooks) as part of the notebook.

All of these are great but I couldn't find a tutorial for building a simple widget from scratch that uses an external javascript library as part of the Jupyter notebook which will be covered in this tutorial.

Source code can be found on [github](https://github.com/kazuar/jupyter_britecharts_widget_tutorial).

## Create dev environment for the new widget

For this tutorial we will need the jupyter package and ipywidgets for our jupyter notebook and widgets and also cookiecutter for the Jupyter notebook widget template.
...
I think that the best way to start this would be to use a pre-defined template for our widget.
This template will already have most of what we will need for our widget.
...
Once you're done you'll have the following folders structure under your new folder (jupyter_britecharts_widget_tutorial):
...
As you can see from the folders tree, the widget has two parts which are the python code and the javascript code.
In order to run the widget we will run python code in our notebook and that code will sync with the javascript code that will render the output in our notebook.

## Running the widget example

The cookiecutter jupyter widget has an integrated "hello world" example.
Just for a sanity check, let's try to run this hello world example.

In order to run this example, we need to install the package in our virtual env and enable it for our jupyter notebook.
...
The packages dependencies are set in [js/package.json](https://github.com/kazuar/jupyter_britecharts_widget_tutorial/blob/master/js/package.json) file.
[package.json](https://github.com/kazuar/jupyter_britecharts_widget_tutorial/blob/master/js/package.json) file is being used by [setup.py](https://github.com/kazuar/jupyter_britecharts_widget_tutorial/blob/master/setup.py) and during the build step it will install [package.json](https://github.com/kazuar/jupyter_britecharts_widget_tutorial/blob/master/js/package.json) dependencies using npm.
...
When you'll call the widget in the notebook, ths python module will call the javascript model which will eventually be rendered in the browser.
...
Create a new python file with the name "britecharts_jupyter_widget.py" in the "<package_name>\<package_name>" folder, "jupyter_britecharts_widget_tutoria\jupyter_britecharts_widget_tutoria" in our example (same folder with the file "example.py").
...
Because we want that our widget will be displayed in the Jupyter notebook, our class must inherit from "ipwidgets.DOMWidget".
Once we inherit from "DOMWidget", we will need to associate it with the front-end with the following members:
...
Each attribute will be a traitlets object that will have the "sync=True" for handling the synchonization of the value with the browser.

We also want to add another attribute that will save our "_model_data" for the bar chart.
...
1. _default_layout - will control the layout of the output cell and make enough space for the bar chart
2. set_data - allow the user to set his own data for the bar chart
...
The last part will be to create the front-end side of our widget.
The code will create two objects:

1. widget model - the widget model is only defining some default values that can be overriden by the python class.
...
We start by importing jupyter-js-widgets, d3, britecharts bar chart, css, etc.
...
The widget view will handle the rendering the bar chart in the browser.
"this.elm" is the element that we use d3 to convert it into the container that will hold the bar chart.
After we are done setting up the bar chart object we combine it with our data in the bar char container.

The settings for the bar chart can be changed by your preferences, we also add a color scheme for the bar chart that we've imported earlier on. For now, to make things simpler, we will hard code all these settings in the javascript file.
...
We're ready to run the full example.

First, let's unintall the plugin from our jupyter notebook so we can clean the environment before re-building and re-installing it.

{% highlight bash %}
jupyter nbextension uninstall --py --sys-prefix jupyter_britecharts_widget_tutorial
{% endhighlight %}

Next step will be to delete the static folder that is being generated by the "setup.py build" step.
...
Re-install the widget and enable it on our jupyter notebook:
...
Run the code in the cell and you see the barchart in the output cell of the notebook.

![jupyter_widget]({{ site.baseurl }}/images/jupyter_widget/jupyter_widget.png)

## Debugging

Debugging the jupyter can be a little tricky as there are many components and you need to know where to look in order to find the error.

I use these tools to debug my Jupyter widgets:

1. If it's an issue with the Python code, you will be able to see the error in the Jupyter notebook console output.
2. If it's an issue with the Javascript code, you will either find the error during the "setup.py build" step or in the browser using the browser's developer tools -> javascript console, firebug, etc.

## Summary

This was a very basic example of creating a Juyper notebook widget that uses external javascript libraries.
Britecharts was used for this example just as an external javascript library and the same method can be used with other external javascript libraries.
